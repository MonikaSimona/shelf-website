    ...
    comments: [
      {
        ...
        userName: "janeweyn",
        userImage:
          "https://images.unsplash.com/photo-1563306406-e66174fa3787?ixid=MnwxMjA3fDB8MHxzZWFyY2h8OHx8Z2lybHxlbnwwfHwwfHw%3D&ixlib=rb-1.2.1",
        date: "12.03.2021",
        comment:
          "In commodo rhoncus eros, eu condimentum magna mollis in. Fusce egestas velit non justo aliquam ultrices. Interdum et malesuada fames ac ante ipsum primis in faucibus. Aenean nec blandit leo. In tempus hendrerit cursus. Morbi sed sapien gravida, vulputate diam quis, tempus nunc. Etiam et ipsum semper, accumsan enim et, gravida mi. ",
      },
      {
        displayName: "Erik Voss",
        userName: "erikvoss",
        userImage:
          "https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1",
        date: "21.08.2021",
        comment:
          "In commodo rhoncus eros, eu condimentum magna mollis in. Fusce egestas velit non justo aliquam ultrices. Interdum et malesuada fames ac ante ipsum primis in faucibus. Aenean nec blandit leo.s",
      },
      {
        displayName: "Jane",
        userName: "janeweyn",
...
export const reviewItemsSlice = createSlice({
  ...
  reducers: {
    ...
    findItemById: (state, action: PayloadAction<Params>) => {
      let filtered;
      console.log("BY ID", action.payload.id);
      if (action.payload.type === "books") {
        filtered = books;
      } else if (action.payload.type === "tvshows") {
        filtered = tvshows;
      ...
      filtered = filtered.filter((review) => review.id === action.payload.id);
      state = filtered;
      console.log("STATE", state);
      return state;
    },
    findCommentsByReview: (state: any, action: PayloadAction<Params>) => {
      let filtered;
      if (action.payload.type === "books") {
        filtered = books;
      } else if (action.payload.type === "tvshows") {
      ...
      state = filtered[0] && filtered[0].comments;
      return state;
    },
  },
...
export const { getAllItems, filterItems, findItemById, findCommentsByReview } =
  reviewItemsSlice.actions;
